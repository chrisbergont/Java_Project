...
public class PanneauPrincipalTask extends JPanel // nouvelle fenetre pour création de tâche
{
	...
	public JDatePicker getCalendar() // accesseur calendrier
	{
		if (calendar == null) // si y'a pas de calendrier
		{
			...
			model.setDate(cal.get(Calendar.YEAR), cal.get(Calendar.MONTH), cal.get(Calendar.DAY_OF_WEEK)); // on recupère la bonne période
	...
	public PanneauPrincipalTask() // constructeur
	{
		...
		GridLayout grille = new GridLayout(2, 0, 10, 10); // on lui crée une grille (nb lignes, nb colonnes, horizontal
															// gap, vertical gap)
